...
for i in range(5):
    ...
    if agent.goal_node:
        ...
        path_length = 0
        node = agent.goal_node
        while node is not None:
            path_length += 1
            node = node.parent
        print(f"Reached goal on attempt {i + 1}!")
        print(f"Time taken: {time_taken}")
        print(f"Path length: {path_length}")
        print(f"Number of nodes explored: {len(agent.explored)}")
        print(f"Number of nodes in frontier: {len(agent.frontier)}")
